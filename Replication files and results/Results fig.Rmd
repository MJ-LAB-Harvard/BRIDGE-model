...
raw_data <-  read_excel("C:\\Users\\zh096\\Mass General Brigham Incorporated\\Lab Hub - Documents\\Projects\\MJ_NIH model\\Writing and presentation\\2. Model Paper\\Journal version\\Figures\\Data for result figure.xlsx", sheet = "Results")
...
df_long <- df %>% 
  ...
  mutate(
    ...
    Scenario = case_when(
      ...
      str_detect(variable, regex("\\blow\\b",      ignore_case = TRUE)) ~ "No-increase",          
...
df_long <- bind_rows(
  ...
  df_long %>% filter(Scenario == "No-increase")            %>% split_future(),   
...
# ──────────────────────────────────────────────────────────
# helper to suppress all guides in a ggplot object
# ──────────────────────────────────────────────────────────
...
pct_axes <- tribble(
  ~entity,     ~y_min, ~y_max, ~y_breaks,                ~y_label,              ~plot_title,
  "PI",           -75,     10,   seq(-75, 10, 20),        "% change from base", "Principal Investigators with active NIH grants",
  ...
  "Papers",       -75,     10,   seq(-75, 10, 20),        "% change from base", "NIH-supported fundamental research papers",
  "Drugs",        -75,     10,   seq(-75, 10, 20),        "% change from base", "Annual novel drugs approved",
  "Cumulative",   -75,     10,   seq(-75, 10, 20),        "% change from base", "Cumulative novel drugs shortfalls"
)
...
supp_data <-  read_excel("C:\\Users\\zh096\\Mass General Brigham Incorporated\\Lab Hub - Documents\\Projects\\MJ_NIH model\\Writing and presentation\\2. Model Paper\\Journal version\\Figures\\Data for result figure.xlsx", sheet = "Supp")

names(supp_data)[1] <- "variable"
...
df_supp <- supp_data %>%
  ...
  mutate(
    ...
    entity = str_trim(
      str_remove(
        variable,
        regex(
          "\\s+(low|cut|ramp up in 1yr|ramp up in 4yr|base)\\s*$",
          ignore_case = TRUE
        )
      )
    )
  ...
  filter(!is.na(Scenario)) %>%
  select(entity, Scenario, year, value, variable) %>%
  arrange(entity, Scenario, year)

# Optional check
unique(df_supp$entity)
...
# =========================================================
# AXIS / LABEL CONTROL TABLE
# EDIT THIS SECTION
# =========================================================

supp_axes <- tribble(
  ~entity,                 ~y_min,  ~y_max, ~y_breaks,                    ~y_label,        ~plot_title,
  "time funded as PI",           0,      25, seq(0, 25, 5),               "Year",         "C) Time funded as PI",
  "lost funding years",      -3,       0, seq(-3, 0, 1),          "Year",    "B) Average funding years lost for surviving PIs",
  "cumulative PI years",  -500000,       0, seq(-500000, 0, 100000),      "PI*Year", "A) Cumulative PI years lost"
)
...
line_entity <- "time funded as PI"
bar1_entity <- "lost funding years"
bar2_entity <- "cumulative PI years"
...
ax_A <- supp_axes %>%
  filter(entity == line_entity)

plot_A <- ggplot(
  df_supp %>% filter(entity == line_entity),
  aes(
    x = year,
    y = value,
    colour = Scenario,
  ...
  scale_y_continuous(
    limits = c(ax_A$y_min, ax_A$y_max),
    breaks = ax_A$y_breaks[[1]],
    labels = label_comma()
  ) +
  labs(
    title = ax_A$plot_title,
    x = NULL,
    y = ax_A$y_label
  ) +
  theme_classic() +
...
# =========================================================
# PANEL B: LOST FUNDING YEARS SHORTFALL
# =========================================================

ax_B <- supp_axes %>%
  filter(entity == bar1_entity)
...
plot_B <- ggplot(bar1_df, aes(x = Scenario)) +
  geom_col(
    aes(y = shortfall, fill = Scenario),
    width = 0.7,
    show.legend = FALSE
  ) +
  scale_y_continuous(
    limits = c(ax_B$y_min, ax_B$y_max),
    breaks = ax_B$y_breaks[[1]],
    expand = c(0, 0),
    labels = comma
  ) +
  scale_x_discrete(position = "top") +
...
# =========================================================
# PANEL C: CUMULATIVE PI YEARS SHORTFALL
# =========================================================

ax_C <- supp_axes %>%
  filter(entity == bar2_entity)
...
plot_C <- ggplot(bar2_df, aes(x = Scenario)) +
  geom_col(
    aes(y = shortfall, fill = Scenario),
    width = 0.7,
    show.legend = FALSE
  ...
  scale_y_continuous(
    limits = c(ax_C$y_min, ax_C$y_max),
    breaks = ax_C$y_breaks[[1]],
    expand = c(0, 0),
    labels = comma
  ) +
  scale_x_discrete(position = "top") +
...
supp_final <- wrap_plots(
  list(plot_C, plot_A, plot_B),
  ncol = 1
) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")
...
ggsave(
  filename = "supplement_figure.svg",
  plot = supp_final,
  width = 11,
  height = 7,
  units = "in",
